...
namespace Jinkong.Wx
{
    public interface IWxPay
    {
        ...
        /// <returns></returns>
        Task<PrePayResult<AppPayResult>> BuildApp(string appId, string mchId, string mchKey, string title,
            int totalAmount,
        ...
        /// <param name="expire"></param>
        /// <returns></returns>
        Task<PrePayResult<H5PayResult>> BuildH5(string appId, string mchId, string mchKey, string title,
            int totalAmount,
        ...
        /// <param name="expire"></param>
        /// <returns></returns>
        Task<PrePayResult<JsApiResult>> BuildJsApi(string appId, string mchId, string mchKey, string title,
            int totalAmount,
        ...
        /// <param name="expire"></param>
        /// <returns></returns>
        Task<PrePayResult<QrCodeResult>> BuildWxPayQrCode(string appId, string mchId, string mchKey, string title,
            int totalAmount,
        ...
        /// <param name="notifyUrl"></param>
        /// <returns></returns>
        Task<(bool success, string originXml)> Refund(string appId, string mchId, string mchKey, string localTranNo,
        ...
        public RefundQueryResultInner(string resultXml) : base(resultXml)
        {
            ...
            for (int i = 0; i < refundNoList.Count; i++)
            {
                List.Add(new WxRefundListData
                {
                    ...
                    SuccessTime = time[i].IsNullOrWhiteSpace()
                        ? null
                        : (long?) time[i].ParseTo<DateTime>().GetLongDate()
                });
            }
        }
    ...
    public class WxRefundData
    {
        ...
        public long TotalRefundAmount =>
            List?.Where(r => r.Status == WxRefundListData.RefundStatus.SUCCESS)?.Sum(r => r.RefundAmount) ?? 0;

        /// <summary>
        /// 总已退款实际金额(去掉代金券之类的)
        /// </summary>
        public long TotalRealRefundAmount => List?.Where(r => r.Status == WxRefundListData.RefundStatus.SUCCESS)
            ?.Sum(r => r.RealRefundAmount) ?? 0;
    }
