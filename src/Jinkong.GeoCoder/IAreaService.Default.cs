...
namespace Jinkong.GeoCoder
{
    class DefaultAreaService : IAreaService, Shashlik.Kernel.Dependency.ISingleton
    {
        ...
        private static Lazy<IReadOnlyDictionary<string, AreaItems>> _areas =
            new Lazy<IReadOnlyDictionary<string, AreaItems>>(() => LoadAreas());
        ...
        public async Task<AreaModel> ConvertToAreaModel(string address)
        {
            ...
            try
            {
                res = await HttpHelper.GetString(
                    $"https://restapi.amap.com/v3/geocode/geo?address={address}&key={GeoCoderOptions.Value.Key}");
                ...
                foreach (var p in DefaultAreaService.Areas)
                {
                    foreach (var c in p.Value.Children)
                    {
                        ...
                        {
                            return new AreaModel {AreaCode = code, CityCode = c.Key, ProvinceCode = p.Key};
                        }
                    }
                }
        ...
        public async Task<AreaModel> ConvertToAreaModel(decimal lat, decimal lng)
        {
            ...
            {
                res = await HttpHelper.GetString(
                    $"https://restapi.amap.com/v3/geocode/regeo?location={lng},{lat}&key={GeoCoderOptions.Value.Key}&extensions=base");
                var resObj = JsonConvert.DeserializeObject<JObject>(res);
        ...
        public AreaModel Get(string areaCode)
        {
            ...
            foreach (var province in Areas)
            {
                foreach (var city in province.Value.Children)
                {
                    ...
                    {
                        computedDic.Add(area.Key,
                            new AreaModel {AreaCode = area.Key, CityCode = city.Key, ProvinceCode = province.Key});
                    }
                }
            }
        ...
        public GeoRectangle GetRectangle(GeoPoint center, decimal distance)
        {
            ...
            var longitudeConversionFactor = distance / _latitudeDistanceInKilometersDivisor /
                                            Convert.ToDecimal(
                                                Math.Abs(Math.Cos(Convert.ToDouble(center.Lat) * (Math.PI / 180))));
        ...
        static IReadOnlyDictionary<string, AreaItems> LoadAreas()
        {
            ...
            foreach (var provinceItem in provinceList)
            {
                ...
                foreach (var cityItem in cities)
                {
                    ...
                    {
                        AreaItems area = new AreaItems {Value = areaItem.Key, Label = areaItem.Value,};
