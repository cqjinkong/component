...
namespace Jinkong.Bank
{
    ...
    internal class DefaultBankService : IBankService
    {
        ...
        {
            cardModel = new BankCardModel {CardType = CardType.Unknown};
            ...
            try
            {
                var resultStr =
                    HttpHelper.GetString(
                            $"https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo={bankCardNo}&cardBinCheck=true")
                        .GetAwaiter().GetResult();

                var obj = JsonConvert.DeserializeObject<JObject>(resultStr);
