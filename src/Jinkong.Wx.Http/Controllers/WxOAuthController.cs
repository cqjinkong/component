...
using Jinkong.Wx.Notifies;
using Microsoft.Extensions.DependencyInjection;
using Jinkong.AspNetCore;
using Microsoft.Extensions.Options;
using Jinkong.Utils.Extensions;

namespace Jinkong.Wx.Controllers
{
    /// <summary>
    /// 微信授权登录
    /// </summary>
    ...
    public class WxOAuthController : ControllerBase
    {
        ...
        public IActionResult snsapi_base(string returnUrl, string type, string attach)
        {
            ...
            if (!Jinkong.Utils.Extensions.StringExtensions.IsUrl(validReturnUrl))
                return Content("回调地址参数错误");

            // 当前api主域
            string host = Options.Value.ThisHost.TrimEnd('/');
            string callback = $"{host}/wxoauth/snsapi_base/callback";
            callback = Jinkong.Utils.Extensions.StringExtensions.UrlArgsCombine(callback, new List<KeyValuePair<string, object>>
            {
        ...
        public async Task<IActionResult> snsapi_baseOAuthCallback(string code, string state, string returnUrl, string type, string attach)
        {
            try
            {
                ...
                returnUrl = Jinkong.Utils.Extensions.StringExtensions.UrlArgsCombine(returnUrl, extendArgs);
