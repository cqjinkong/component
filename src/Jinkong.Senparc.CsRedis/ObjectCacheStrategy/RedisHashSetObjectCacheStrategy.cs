...
namespace Jinkong.Senparc.CsRedis
{
    ...
    //where TContainerBag : class, IBaseContainerBag, new()
    {
        ...
        public static RedisHashSetObjectCacheStrategy Instance { get; private set; }

        /// <summary>
        /// Redis 缓存策略
        /// </summary>
        internal RedisHashSetObjectCacheStrategy(CSRedisClient cSRedisClient) : base(cSRedisClient)
        {
            if (Instance != null)
                throw new Exception("RedisObjectCacheStrategy不能重复初始化");
            Instance = this;
        }
